...
for WHAT in (INPUT, OUTPUT):
    ...
    data, psize = rectify_sample(data, dr, theta, interp=1, find_order=True)
    ...
    for i, offset in enumerate(depths):
        phase = np.angle(data[N//2+offset])
        diff = np.diff(phase, axis=0)
        aim = ax[0, i].imshow(np.abs(data[N//2+offset]), vmax=dmax*.7, vmin=0, cmap='viridis', extent=exts)
